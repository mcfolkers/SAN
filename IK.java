...
public class IK {
	...
	private static void handJointCalculation(GripperPosition pos, JointValues j) {
		...
		j.pitch = -90; /*
						 * Default value of the pitch is always -90, since the
						 * gripper is always faced downwards
						 */
		j.grip = pos.grip; /*
							 * Default value of the gripper, obtained from
							 * GripperPoition
							 */
	}
	...
	private static void armJointCalculation(Point wristCoords, JointValues j) {
		...
		j.zed = 364 + z; /*
						 * the height of the zed is the height of the
		 ...
		 */
		j.shoulder = Math.toDegrees((Math.atan2(x, y) - (Math.atan2(253.5 * s2,
				253.5 + 253.5 * c2))));
	...
	private static void inverseKinematics(Vector<GripperPosition> p,
		...
		for (int i = 0; i < p.size(); i++) {
			...
			/*
			 * To prevent errors, the arm will switch position when the next
			 * move is out of range. We improved the code to prevent unnecessary
			 * armswitches
			 */
